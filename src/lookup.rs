...
use crate::schema::{PathPart, SchemaSpec, resolve_path_dtype, to_mmdb_path};
...
pub fn output_dtype(kwargs: &LookupPathKwargs) -> PolarsResult<SchemaSpec> {
    ...
    if !dtype.is_scalar() {
        polars_bail!(
            InvalidOperation:
            "nested MMDB path output is not available until the schema-guided builder is selected; got {}",
            dtype.to_polars()
        )
    }
    Ok(dtype)
...
struct LookupBatch<'a> {
    unique: Vec<LookupResult<'a, Vec<u8>>>,
...
{
    let batch = lookup_batch(ips, reader, kwargs)?;
...
mod tests {
    ...
    fn deduplicates_records_before_decoding() {
        ...
        let batch = lookup_batch(ips.str().unwrap(), &reader, &kwargs).unwrap();
